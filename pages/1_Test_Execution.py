...
# ---------------------------------------------------------------------------
# Duration trend — are tests getting slower?
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Test runs table — filterable + sortable, status badges
# ---------------------------------------------------------------------------
...
status_options = sorted(tests["status"].dropna().astype(str).str.lower().unique().tolist())
...
if name_query.strip():
    ...
    table_df = table_df[table_df["test_name"].astype(str).str.lower().str.contains(q, na=False)]
...
view = table_df[display_cols].sort_values("timestamp", ascending=False).reset_index(drop=True)
...
st.dataframe(
    ...
    column_config={
        ...
        "duration_sec": st.column_config.NumberColumn("duration_sec", format="%.2f s"),
